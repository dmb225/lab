...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app lab db upgrade
	@echo ""
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app lab db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app lab db upgrade
...
db-init: docker-db
	...
	@sleep 8
	cd backend && uv run lab db migrate -m "initial" || true
	cd backend && uv run lab db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend lab db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend lab db upgrade

db-downgrade:
	uv run --directory backend lab db downgrade

db-current:
	uv run --directory backend lab db current

db-history:
	uv run --directory backend lab db history

# === Server ===
run:
	uv run --directory backend lab server run --reload

run-prod:
	uv run --directory backend lab server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend lab server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend lab user create-admin

user-create:
	uv run --directory backend lab user create

user-list:
	uv run --directory backend lab user list
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/lab"
